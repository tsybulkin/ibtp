...
-module(netw).
-export([gen_rand_net/1, setup/2
		]).

-define(PORTS_NBR,4).
-define(MAX_RAND_ATTEMPTS,5).
-define(BPS,1000). % 1000 bit per second
...
gen_rand_net(N) ->
	...
	Net = dict:from_list([ {box_name(J),[]} || J <- lists:seq(1,N) ]),
	wire_boxes(N,M,Net).



box_name(J) -> "box "++ integer_to_list(J).
...
wire_boxes(N,M,Net) ->
	case get_random_wire(N,Net,0) of
		{Box1,Box2} ->
			...
			Net1 = dict:append(Box1,{Box2,P1},dict:append(Box2,{Box1,P2},Net)),
...
get_random_wire(N,Net,Attempt) ->
	B1 = box_name(random:uniform(N)),
	B2 = box_name(random:uniform(N)),
